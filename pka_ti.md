...
7) as lmp file build with atomsk only care about the different between element, in case for the same model that know 1 2 3 4 for X F O H, while you only have O H (for hydronium), just create a random element for X and F so the lmp will be ordered as 1 for X, 2 for F, etc, once the lmp is build, you can delete it from lammps and renumber it. Place it after read_file 
```bash
group fake type 1 2
delete_atoms group fake compress yes

reset_atoms id   # optional but recommended in many workflows
```
